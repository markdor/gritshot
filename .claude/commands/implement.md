...
- Reihenfolge sinnvoll wählen (z. B. Schema/Migration → Server-Modul/Action → UI → Tests, oder TDD-Reihenfolge falls passend),
- CLAUDE.md-Konventionen respektieren: `FileValidationError` mit `userMessage` für erwartbare Fehler, pino-Logging statt `console.*`, Vitest-Projektaufteilung (client/server), Playwright nur bei kritischen Flows, neue user-facing Strings über `m.xxx()` in beiden Message-Dateien (en + de),
